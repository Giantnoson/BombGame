...
using System.Collections.Generic;
    ...
    public class MapScan : MonoBehaviour
    {
        ...
        public void ScanArea(Vector2Int refPos,int refOffset)
        {
            ...
            print("扫描中心坐标:" + refPos.x + "," + refPos.y + "\n扫描范围:x = " + startX + "," + endX + ", z = " + startZ + "," + endZ);
            Vector3 pos = new Vector3(0 , startY, 0);
        ...
        {
            bool flag = false;
            int colliderCount = Physics.OverlapBoxNonAlloc(pos, new Vector3(0.4f, 0.4f, 0.4f),_hitColliders);
            ...
            if (!flag)
            {
                MapData.Map[i,j] = TagPointPairsMap[nameof(ObjectType.Nothing)];//当无标签时
            }
        }
        ...
        {
            pos.y = startY;
            Vector2Int point = GetVirtualCoord(pos);
            bool flag = false;
            int colliderCount = Physics.OverlapBoxNonAlloc(pos, new Vector3(0.4f, 0.4f, 0.4f),_hitColliders);
            for (int k = 0; k < colliderCount; k++)
            {
            ...
            if (!flag)
            {
                MapData.Map[point.x,point.y] = TagPointPairsMap[nameof(ObjectType.Nothing)];//当无标签时
            }
        }
        ...
        public void ScanPoint(Vector2Int pos)
        {
            ...
            if (!flag)
            {
                MapData.Map[pos.x,pos.y] = TagPointPairsMap[nameof(ObjectType.Nothing)];//当无标签时
            }
        }
        ...
        public List<PointStepTracker> ExistPath(Vector3 startPos, Vector3 endPos, int maxArea)
        {
            ...
            {
                Vector2Int startVirtualCoord = GetVirtualCoord(startPos);
                Vector2Int endVirtualCoord = GetVirtualCoord(endPos);
        ...
        public List<PointStepTracker> ExistPath(Vector2Int startVirtualPos, Vector2Int endVirtualPos, int maxArea)
        {
            ...
            var count = 1;
            for (int i = 0; i < maxArea; i++)//最大范围控制
            {
                var bfsCount = count;
                count = 0;
                for (int j = 0; j < bfsCount; j++)//单次
                {
            ...
            while (que.Count > 0)
            {
                ...
                foreach (var direction in directions)
                {
                    ...
                    if (!visited.Contains(next) && IsWalkable(next))
                    {
                        if (next == endVirtualPos)
                        {
                            return new PointStepTracker(next, count + 1);
                        }
                        que.Enqueue(next);
                    }
                    visited.Add(next);
        ...
        /// <param name="maxArea">最大步长</param>
        /// <returns>返回起始位置到布标位置包含步长的列表</returns>
        public List<PointStepTracker> ExistPathForFindPlayer(Vector2Int startVirtualPos, Vector2Int endVirtualPos, int maxArea)
        {
            if (!(IsValidPosition(startVirtualPos) && IsValidPosition(endVirtualPos)))
            {
                Debug.LogError("不合法的位置: " + startVirtualPos + " or " + endVirtualPos);
        ...
        public List<PointStepTracker> SearchTagWithDFS(Vector2Int startVirtualPos, ObjectType tag, int maxStep)
        {
            ...
            List<PointStepTracker> result = new List<PointStepTracker>();
        ...
        public PointStepTracker GetRandomPointInArea(Vector2Int startPos, int area)
        {
            ...
            var maxx = Mathf.CeilToInt(MathF.Min(Mathf.Min(startPos.x + area - 1, offsetDistance * 2), MapData.Map.GetLength(0)));
            var miny = Mathf.CeilToInt(MathF.Min(Mathf.Max(startPos.y - area + 1, 0),offsetDistance * 2));
            var maxy = Mathf.CeilToInt(MathF.Min(Mathf.Min(startPos.y + area - 1, offsetDistance * 2), MapData.Map.GetLength(1)));
            ...
            while (tryCount > 0)
            {
                ...
                if (IsWalkable(point))
                {
                    return new PointStepTracker(point,0);
                }
                tryCount--;
            }
