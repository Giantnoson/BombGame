...
namespace GameSystem.Character.Enemy
{
    ...
    public abstract class EnemyAIBaseState : FsmState<EnemyAIController>
    {
        ...
        ///     检查当前位置是否在爆炸范围内
        /// </summary>
        protected bool IsInExplosionRange(Vector3 position)
        {
            return Owner.IsInExplosionRange(position);
        }
        ...
        {
            if (Owner != null) Owner.MoveTo(Owner.MapInfo.SearchPath(Owner.transform.position, position, true));
        }
    }
}