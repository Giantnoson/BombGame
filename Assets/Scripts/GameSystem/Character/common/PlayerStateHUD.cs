...
namespace GameSystem.Character
{
    public class PlayerStateHUD : MonoBehaviour
    {
        ...
        public void LoadHUD(HUDEvent.LoadHUDEvent evt)
        {
            ...
            // 初始化经验值相关
            print( $"EXP: {evt.EXP}/{evt.GlobalProper.maxExpToLevelUp} fill = {(float)evt.EXP / evt.GlobalProper.maxExpToLevelUp}");
            expText.text = $"{evt.EXP}/{evt.GlobalProper.maxExpToLevelUp}";
            expBar.fillAmount = (float)evt.EXP / evt.GlobalProper.maxExpToLevelUp;
    
            // 初始化炸弹相关
            bombCountText.text = $"{evt.GlobalProper.initBombCount.ToString()}/{evt.CharacterProper.maxBombCount.ToString()}({evt.CharacterProper.bombRecoveryTime.ToString("F2")})";
        ...
        { 
            ...
            bombCountText.text =$"{evt.BombCount.ToString()}/{evt.MaxBombCount.ToString()}({evt.BombRecoveryTime.ToString("F2")})";
        }
